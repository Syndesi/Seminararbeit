 ...
 *
 * @method     ChildUserQuery leftJoinAuthorized($relationAlias = null) Adds a LEFT JOIN clause to the query using the Authorized relation
 * @method     ChildUserQuery rightJoinAuthorized($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Authorized relation
 * @method     ChildUserQuery innerJoinAuthorized($relationAlias = null) Adds a INNER JOIN clause to the query using the Authorized relation
 *
 * @method     ChildUserQuery joinWithAuthorized($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Authorized relation
 *
 * @method     ChildUserQuery leftJoinWithAuthorized() Adds a LEFT JOIN clause and with to the query using the Authorized relation
 * @method     ChildUserQuery rightJoinWithAuthorized() Adds a RIGHT JOIN clause and with to the query using the Authorized relation
 * @method     ChildUserQuery innerJoinWithAuthorized() Adds a INNER JOIN clause and with to the query using the Authorized relation
 *
 * @method     \AuthorizedQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 * @method     ChildUser findOne(ConnectionInterface $con = null) Return the first ChildUser matching the query
 * @method     ChildUser findOneOrCreate(ConnectionInterface $con = null) Return the first ChildUser matching the query, or a new ChildUser object populated from the query conditions when no match is found
 *
 * @method     ChildUser findOneById(int $id) Return the first ChildUser filtered by the id column
 * @method     ChildUser findOneByForename(string $forename) Return the first ChildUser filtered by the forename column
 ...
 * @method     ChildUser findOneByEmailVerified(boolean $email_verified) Return the first ChildUser filtered by the email_verified column
...
abstract class UserQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, forename, surname, email, email_verified, hash, created_at, updated_at FROM user WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query by a related \Authorized object
     *
     * @param \Authorized|ObjectCollection $authorized the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildUserQuery The current query, for fluid interface
     */
    public function filterByAuthorized($authorized, $comparison = null)
    {
        if ($authorized instanceof \Authorized) {
            return $this
                ->addUsingAlias(UserTableMap::COL_ID, $authorized->getUserId(), $comparison);
        } elseif ($authorized instanceof ObjectCollection) {
            return $this
                ->useAuthorizedQuery()
                ->filterByPrimaryKeys($authorized->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByAuthorized() only accepts arguments of type \Authorized or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the Authorized relation
     *
     ...
     * @return $this|ChildUserQuery The current query, for fluid interface
     */
    public function joinAuthorized($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('Authorized');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'Authorized');
        }
    ...
    /**
     * Use the Authorized relation Authorized object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \AuthorizedQuery A secondary query class using the current class as primary query
     */
    public function useAuthorizedQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinAuthorized($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Authorized', '\AuthorizedQuery');
    }
