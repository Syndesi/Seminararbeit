...
abstract class DwdCloudines implements ActiveRecordInterface
{
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'quality':
                        ...
                        break;
                    case 'v_n_i':
                        $stmt->bindValue($identifier, $this->v_n_i, PDO::PARAM_STR);
                        break;
                    case 'v_n':
                        $stmt->bindValue($identifier, $this->v_n, PDO::PARAM_INT);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[4], $arr)) {
            $this->setVNI($arr[$keys[4]]);
        }
        if (array_key_exists($keys[5], $arr)) {
            $this->setVN($arr[$keys[5]]);
        }
    }
